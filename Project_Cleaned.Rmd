...
```{r variable_exploration1, message = FALSE, warning = FALSE}
data %>%
  select(outcome_type) %>%
  distinct(.)
```
...
```{r variable_exploration2, message = FALSE, warning = FALSE}
data %>%
...
```{r variable_exploration3, message = FALSE, warning = FALSE}
data %>%
...
```{r variable_exploration4, message = FALSE, warning = FALSE}
data %>%
  select(intake_type) %>%
  distinct(.)
```
...
```{r variable_exploration5, message = FALSE, warning = FALSE}
data %>%
...
```{r variable_exploration6, message = FALSE, warning = FALSE}
data %>%
...
```{r variable_exploration7, message = FALSE, warning = FALSE}
data %>%
...
```{r variable_exploration8, message = FALSE, warning = FALSE}
data %>%
  filter(animal_type == "Dog") %>%
  select(color) %>%
  distinct(.)
...
```{r distributional_exploration1, message = FALSE, warning = FALSE}
data_dog %>%
...
```{r distributional_exploration2, message = FALSE, warning = FALSE}
data_dog %>%
...
```{r distributional_exploration3, message = FALSE, warning = FALSE}
data_dog %>%
  ...
  arrange(animal_id_intake) %>%
  ggplot(aes(x = times, y = count)) +
  geom_bar(stat = "identity", fill = "dodgerblue") +
  facet_grid(. ~ outcome_type) +
  labs(x = "Lifetime Visits To Shelter Per Animal", y = "Outcome Count", title = "It is very uncommon for dogs that are past their initial visit to the shelter\nto be transfered") +
  theme_minimal()
```
...
```{r distributional_exploration4, message = FALSE, warning = FALSE}
data_dog %>%
  filter((outcome_type == "Adoption" | outcome_type == "Transfer" | outcome_type == "Return to Owner" | outcome_type == "Euthanasia")) %>%
...
```{r distributional_exploration5, message = FALSE, warning = FALSE}
data_dog %>%
...
```{r distributional_exploration6, message = FALSE, warning = FALSE}
data_dog %>%
  filter((outcome_type == "Adoption" | outcome_type == "Transfer" | outcome_type == "Return to Owner" | outcome_type == "Euthanasia") & (sex_upon_outcome != "NULL")) %>%
...
```{r distributional_exploration7, message = FALSE, warning = FALSE}
data_dog %>%
...
ggplot(aes(x=time_in_shelter_days, y=as.factor(outcome_type), fill=factor(..quantile..))) +
  ...
  scale_fill_viridis(discrete = TRUE, name = "Quartiles", option = "D") +
...
```{r monthofyear, message = FALSE, warning = FALSE}
data_dog %>%
  filter((outcome_type == "Adoption" | outcome_type == "Transfer" | outcome_type == "Return to Owner" | outcome_type == "Euthanasia")) %>%
...
```{r data_cleaning, message = FALSE, warning = FALSE, results = 'hide'}
...
```{r breedgroups, message = FALSE, warning = FALSE}
data_dog_mod %>%
  mutate(outcomeid = 1:length(data_dog_mod$outcometype)) %>%
  gather(key = outcomeid, value = breedgroup, dogtype1, dogtype2, dogtype3) %>%
  filter(!(breedgroup %in% c("One Type", "Two Types"))) %>%
  group_by(breedgroup, outcometype) %>%
  summarise(counts = n()) %>%
  group_by(breedgroup) %>%
  mutate(freq = counts/sum(counts)) %>%
  ggplot(aes(x = breedgroup, y = freq)) +
  geom_bar(stat = "identity", fill = "dodgerblue") +
  scale_y_continuous(labels = scales::percent) +
  facet_wrap(~ outcometype, nrow = 2) +
  coord_flip() +
  labs(x = "Breed Group", y = "Percent of Outcome Total", title = "Almost all adopted dogs are Spayed or Neutered", subtitle = "Male dogs appear to be more likely to be euthanized than female dogs") +
  theme_minimal()
```
...
```{r isblack, message = FALSE, warning = FALSE}
data_dog_mod %>%
  group_by(isblack, outcometype) %>%
  summarise(counts = n()) %>%
  group_by(isblack) %>%
  mutate(freq = counts/sum(counts)) %>%
  ggplot(aes(x = isblack, y = freq)) +
  geom_bar(stat = "identity", fill = "dodgerblue") +
  scale_x_discrete(labels = c("Non-Black", "Black")) +
  scale_y_continuous(labels = scales::percent) +
  facet_wrap(~ outcometype, nrow = 2) +
  coord_flip() +
  labs(x = "Dog Color", y = "Percent of Outcome Total", title = "Little difference shown between outcomes of black dogs and non-black dogs", subtitle = "The theory that black dogs are adopted less often seems statistically unfounded") +
  theme_minimal()
```
...
```{r modelTrain, message = FALSE, warning = FALSE, results = 'hide', eval = FALSE}
...
```{r knnModel, message = FALSE, warning = FALSE}
knn_fit
```

```{r knnAccuracy, message = FALSE, warning = FALSE}
ggplot(knn_fit) +
  theme_minimal()
```
...
```{r rf1Model, message = FALSE, warning = FALSE}
rf_fit
```

```{r rf1Accuracy, message = FALSE, warning = FALSE}
ggplot(rf_fit) +
  theme_minimal() +
  ylim(0.55, 0.8)
```

```{r rf2Model, message = FALSE, warning = FALSE}
rf_fit2
```

```{r rf2Accuracy, message = FALSE, warning = FALSE}
ggplot(rf_fit2) +
  theme_minimal() +
  ylim(0.55, 0.8)
```

```{r rf2Kappa, message = FALSE, warning = FALSE}
